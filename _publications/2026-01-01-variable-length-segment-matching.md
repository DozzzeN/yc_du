---
title: "Physical Layer Secret Key Generation Leveraging Variable-length Segment Matching in Wireless Networks"
collection: publications
category: manuscripts
permalink: /publication/2026-tmc-variable-length
excerpt: 'This paper addresses the trade-off between key generation rate and accuracy by introducing an adaptive variable-length segment matching framework.'
date: 2026-01-01
venue: 'IEEE Transactions on Mobile Computing (TMC)'
paperurl: 'https://ieeexplore.ieee.org/journal/tmc'
citation: 'Yicong Du, Huan Dai, Yuchen Su, Haitao Jia, Shuai Li, Yanzhi Ren, Hongbo Liu. Physical Layer Secret Key Generation Leveraging Variable-length Segment Matching in Wireless Networks[J]. IEEE Transactions on Mobile Computing, 2026. (Early Access).'
---

### Contributions & Highlights
* **Adaptive Segmentation:** Addressed the critical trade-off between key generation rate (KGR) and accuracy caused by fixed-length segmentation in traditional matching-based methods.
* **DTW-based Partitioning:** Proposed a framework that dynamically partitions channel samples based on **Dynamic Time Warping (DTW)** distance to maximize inter-segment distinctiveness.
* **Recursive Retrieval:** Developed a **recursive rapid retrieval mechanism** to efficiently identify consistent patterns, significantly outperforming state-of-the-art methods in robustness and efficiency.
